...
function HomeIcon({ active }: { active: boolean }) {
  return active ? (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
      <path d="M3 10.5L12 3L21 10.5V20C21 20.55 20.55 21 20 21H15V14H9V21H4C3.45 21 3 20.55 3 20V10.5Z" fill="#1E3A8A" />
    </svg>
  ) : (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
...
function AppointmentsIcon({ active }: { active: boolean }) {
  return active ? (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
      ...
      <path d="M8 2V6M16 2V6" stroke="#1E3A8A" strokeWidth="1.8" strokeLinecap="round" />
...
function ProfileIcon({ active }: { active: boolean }) {
  return active ? (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
      ...
      <path d="M4 21C4 17.134 7.582 14 12 14C16.418 14 20 17.134 20 21" fill="#1E3A8A" />
    </svg>
  ) : (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
      ...
      {tabs.map((tab) => {
        ...
        return (
          <button
            key={tab.id}
            onClick={tab.onClick}
            className={`flex flex-col items-center gap-0.5 py-1.5 px-3 rounded-xl transition-all duration-200 relative ${isActive ? 'bg-[#EFF6FF]' : ''}`}
          >
            {/* Active top bar */}
            {isActive && (
              <div className="absolute -top-1.5 left-1/2 -translate-x-1/2 w-6 h-[3px] rounded-full bg-[#1E3A8A]" />
            )}
            <div className="relative">
              <Icon active={isActive} />
            </div>
            <span className={`text-[10px] leading-tight ${isActive ? 'text-[#2563EB] font-bold' : 'text-[#9CA3AF] font-medium'}`}>
              {tab.label}
            </span>
          </button>
        );
